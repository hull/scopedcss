define(function() {
  ...
  CssRule.prototype.formatCssText = function(prefix, cssText) {
    ...
    // Match the Selector only, replace it and return a prefixed version
    var selector = cssText.match(/^.*[^('.*{?')]{/);
    var prefixed = this.formatSelector(prefix, selector[0]);
    return cssText.replace(selector, prefixed);
